...
@Configuration
// 扫描 Mapper 接口并容器管理
@MapperScan(basePackages = DruidShopUserDataSourceConfig.PACKAGE, sqlSessionFactoryRef = "shopuserSqlSessionFactory")
public class DruidShopUserDataSourceConfig {

    //master dao 所在的包
    public static final String PACKAGE = "com.lemon.chen.dao.mapper.shopuser";

    //mapper所在目录
    private static final String MAPPER_LOCATION = "classpath:mapper/shopuser/*.xml";


    @Value("${spring.datasource.druid.two.driver-class-name}")
    private String driverClassName;

    @Value("${spring.datasource.druid.two.url}")
    private String url;
    ...
    //数据源事务管理器
    @Bean(name = "shopuserDataSourceTransactionManager")
    public DataSourceTransactionManager masterDataSourceTransactionManager() {
        return new DataSourceTransactionManager(masterDataSource());
    }

    //创建Session
    @Bean(name = "shopuserSqlSessionFactory")
    public SqlSessionFactory masterSqlSessionFactory(@Qualifier("shopuserDataSource") DataSource masterDataSource) throws Exception {
        ...
        Resource[] mapperLocations = new PathMatchingResourcePatternResolver().getResources(DruidShopUserDataSourceConfig.MAPPER_LOCATION);
